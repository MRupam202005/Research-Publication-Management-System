  ...
  return (
    <div className="flex flex-col min-h-screen bg-slate-50 dark:bg-slate-950 overflow-x-hidden">
      ...
      <section className="relative flex flex-col lg:flex-row w-full min-h-screen shrink-0 overflow-hidden">
        ...
        {/* Right Panel - Glassmorphic Register Form */}
          ...
          <div className="w-full max-w-md bg-white/10 dark:bg-slate-950/30 backdrop-blur-xl border border-white/20 dark:border-white/10 p-8 sm:p-10 rounded-[2rem] shadow-2xl">
            ...
            <form onSubmit={handleSubmit} className="space-y-4">
              ...
              <div className="space-y-1.5 pb-2">
                ...
                <select
                  id="role"
                  className="w-full px-4 py-3 rounded-xl border border-white/20 bg-white/10 text-white focus:border-indigo-400 focus:bg-white/20 focus:ring-2 focus:ring-indigo-400/50 outline-none transition-all shadow-inner appearance-none relative z-10"
                  style={{ colorScheme: 'dark' }}
                  value={role}
                  onChange={(e) => setRole(e.target.value)}
                >
                  {ROLES.map((r) => (
                    <option key={r} value={r} className="text-slate-900 dark:text-slate-200 bg-white dark:bg-slate-900">
                      {r}
                    </option>
                  ))}
                </select>
            ...
            <p className="mt-8 text-center text-sm text-indigo-100/70">
              ...
              <Link href="/login" className="font-bold text-white hover:text-indigo-200 hover:underline transition-all">
                Sign in
              </Link>
            </p>
          </div>
